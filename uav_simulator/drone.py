...
@dataclass
class DroneState:
    """Data class representing complete drone state in NED coordinates"""
...
class Drone:
    def __init__(self, mass: float = 1.0, arm_length: float = 0.25):
        """Initialize drone physical parameters and motor configuration"""
        self.mass = mass 
    ...
    def calculate_torques(self) -> np.ndarray:
        """Calculate roll, pitch, yaw torques from motor thrust differences"""
        l = self.arm_length
        motor_rads = self.motor_speeds * (2 * np.pi / 60)
        motor_thrusts = self.thrust_coeff * (motor_rads ** 2)
        # Roll: differential thrust between left/right motors
        roll_torque = l * ((motor_thrusts[0] + motor_thrusts[3]) - (motor_thrusts[1] + motor_thrusts[2]))
        # Pitch: differential thrust between front/back motors
        pitch_torque = l * ((motor_thrusts[0] + motor_thrusts[1]) - (motor_thrusts[2] + motor_thrusts[3]))
        # Yaw: differential torque from motor rotation directions
    ...
    def get_hover_throttle(self) -> float:
        """Compute throttle value needed to hover"""
